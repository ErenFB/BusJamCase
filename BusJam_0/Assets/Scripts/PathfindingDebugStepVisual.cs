...
public class PathfindingDebugStepVisual : MonoBehaviour {
    ...
    [SerializeField] private List<Transform> visualNodeList;
    ...
    public void Setup(Grid<PathNode> grid) {
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                Vector3 newPos = new Vector3(gridPosition.x, 0, gridPosition.y);
             
                Transform visualNode = CreateVisualNode(newPos);
    ...
    private bool midBool=true;
    private Transform CreateVisualNode(Vector3 position) {
        Transform visualNodeTransform = Instantiate(pfPathfindingDebugStepVisualNode, position, Quaternion.Euler(90,0,0));
      //  visualNodeTransform.SetParent(GameObject.FindGameObjectWithTag("midTransform").transform);
